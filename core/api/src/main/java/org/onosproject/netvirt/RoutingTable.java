...
public class RoutingTable implements TableService {
    ...
    private static final ApplicationId APP_ID = new DefaultApplicationId(6542,
                                                                         "RoutingTable");
    ...
    public FlowRule programDefaultRules(DeviceId dpid) {
        ...
        treatment.transition(pipeline.getNext(MultiTable.ROUTING).tableId);

        FlowRule rule = new NeutronFlowRule(dpid, selector.build(),
                                            treatment.build(),
                                            DEFAULT_PRIORITY, APP_ID, TIME_OUT,
                                            true, MultiTable.ROUTING);
        return rule;
    }
