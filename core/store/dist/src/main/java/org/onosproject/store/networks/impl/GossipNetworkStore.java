 ...
 */
package org.onosproject.store.networks.impl;
...
/**
 * Manages inventory of infrastructure networks using gossip protocol to distribute
 * information.
 */
@Component(immediate = true)
@Service
public class GossipNetworkStore
    ...
    public boolean networkcreate(Iterable<Network> networks) {
        ...
        while (networkors.hasNext()) {
            ...
            if (networkExists(network.id())) {
                log.info("Network id {} is existed", network.id().toString());
                return false;
            }
    ...
    public boolean networkupdate(NetworkId networkId, Iterable<Network> networks) {
        if (!networkExists(networkId)) {
            log.info("Network id {} is not existed", networkId.toString());
            return false;
        }
        Iterator<Network> networkors = networks.iterator();
        while (networkors.hasNext()) {
